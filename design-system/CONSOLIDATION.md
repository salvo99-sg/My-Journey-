...
- [ ] 🔵 **Moduli non in `App.loadModules`**: l'array carica theme/router/storage/modal/timeline/map/search/language/animations, ma **non `State`** né **`Autocomplete`** → i loro `init()` non vengono chiamati dall'App, eppure `theme.js`/`language.js` usano `State.set`. Vanno registrati (State inizializzato prima di Theme; Autocomplete dove serve l'input destinazione)
