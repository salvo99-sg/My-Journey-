...
- [ ] 🔴 **`pwa/404.html`: classi non combacianti** → usa `.button .button--primary`, ma il componente è **`.btn` / `.btn--primary`** (button.css). Anche `.page-404`/`.error-page` non sono stilate da alcun componente. `<link href="/css/app.css">` = **assoluto + bundle inesistente** (base-path + no-build). `lang="en"` (app default IT). Da riscrivere sulle classi reali al cut-over. NB su GitHub Pages il `404.html` va alla **root** per il custom 404
