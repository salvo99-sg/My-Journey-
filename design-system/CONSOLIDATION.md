...
- [ ] 🔵 **Moduli non in `App.loadModules`**: l'array carica theme/router/storage/modal/timeline/map/search/language/animations, ma **non `State`**, **`Autocomplete`**, **`Onboarding`** → i loro `init()` non vengono chiamati dall'App, eppure `theme.js`/`language.js` usano `State.set`. Vanno registrati (State prima di Theme; Onboarding dopo Modal; Autocomplete dove serve l'input destinazione)
